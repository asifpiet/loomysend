...
use Illuminate\Support\Facades\Cache;
	...
	if (!function_exists('filterContactNumber')) { 
    	...
    	function getSMSCamapignContacts($id) {
        ...
        if (!empty($selectedGroups)) {
            foreach ($selectedGroups as $groupId) {
                $groupCounts += Contact::where('group_id', $groupId)->count();
            }
        }
	...
	function getSMSCamapignClicks($id) {
	     $campaign = Campaign::find($id);
	   //  dd($campaign);
	     if($campaign->type == 1){      ///SMS
	         $d = CommunicationLog::where('campaign_id', $id)
	     ...
	     //email
	         ...
	         $d = CommunicationLog::where('campaign_id', $id)
                     ->where(function ($query) {
                         $query->whereNotNull('unique_clicks')
                               ->where('unique_clicks', '>=', 1);
                     })
                     ->count();
            return $d; 
 ...
 */
	...
	if (!function_exists('contact_meta')) {
		...
		function contact_meta(mixed  $status) :string {
			...
			$badges  = [
				ContactAttributeEnum::DATE->value     => "primary-soft",
				ContactAttributeEnum::BOOLEAN->value    => "info-soft",
				ContactAttributeEnum::NUMBER->value   => "danger-solid",
