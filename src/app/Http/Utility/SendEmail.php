...
class SendEmail
{
    ...
    public static function sendGrid($emailFrom, $fromName, $emailTo, $subject, $messages, $emailLog, $credentials): void {
        $baseurl = url('/'); // Base URL including 'xsender'
        $footer = '<div style="border-top:1px solid #ddd; margin-top:20px; padding-top:10px; font-size:12px; color:#888;">
            <p>
                You received this email because you subscribed to our service. 
                If you wish to unsubscribe, click 
                <a href="' . $baseurl . '/user-unsubscribe?email=' . urlencode($emailTo) . '" style="color:#007bff; text-decoration:underline;">
                    Unsubscribe
                </a>.
            </p>
            <p>&copy; ' . date('Y') . ' Sky Vista Consulting. All rights reserved.</p>
             <img src="' . $baseurl . '/email/open/' . $emailLog->id . '" width="1" height="1" alt="Tracking Pixel"  style="display:none;"/>
        </div>';

        // Append the footer to the message body
