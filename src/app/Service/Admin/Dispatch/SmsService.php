...
class SmsService {
    ...
    public function logs($user_id = null, $campaign_id = null) {
        ...
        if ($user_id) {
            ...
            $campaign = Campaign::where('id', $user_id)->first();
            if($campaign){
                $metaDataArry = $campaign->meta_data;
                $selectedGroups = $metaDataArry['contact']['group_ids'];
            }
            $groupContacts = [];
            if (!empty($selectedGroups)) {
                foreach ($selectedGroups as $groupId) {
                        $groupContacts = Contact::whereIn('group_id', $selectedGroups)->pluck('id')->toArray();
                }
            }
            
            if (!empty($groupContacts)) {
                $query->whereIn('contact_id', $groupContacts); // Assuming 'contact_id' in `CommunicationLog` refers to the contact.
            }
        }
        if ($campaign_id) {
    ...
    public function getGroupWhereColumn($column_name, $user_id = null) {
        ...
        return Group::where("user_id", $user_id)->where("status", 2)->whereHas('contacts', function ($query) use ($column_name) {
    ...
    public function store(string $type, array $data, string $contact_type, $user_id = null): array {
        ...
        switch ($contact_type) {
            ...
            case 'file':
                ...
                $meta_data = $this->fileService->readCsv($contacts);
                break;
    ...
    public function saveLog($data) {
        ...
        if(array_key_exists('schedule_at', $data) && $data['schedule_at']) {
            ...
            $data['status'] = (string)CommunicationStatusEnum::DELIVERED->value;
        }
        
        $data['clicked_at'] = ' '; 
        // dd("asdf",$data);
        if ($data && isset($data['contact_id']) && isset($data['campaign_id'])) {
        
            $ifcamapign = CommunicationLog::where([
                'contact_id' => $data['contact_id'],
                'campaign_id' => $data['campaign_id']
            ])->first();
        // 
            // dd($ifcamapign, $data ,$id);
    ...
    public function prepData($data, $meta_data, $gateway, $user_id = null, $update_status = false) {
        ...
        // dd($data, $meta_data, $gateway, $user_id, 'Test');
        if($user_id) {
                ...
                if (($total_credits > $remaining_sms_credits && $user->sms_credit != -1) && !$update_status) {
                    ...
                    if($this->smsGatewayService->hasNestedArray($meta_data)) {
                        ...
                        foreach($meta_data as $key => $value) {
                            ...
                            $data['message'] = $this->getFinalContent($data['message'], $value);
                            
                            if (isset($data['is_default']) && $data['is_default'] === 'on') {
                           
                            $message = $data['message']['message_body'];
                            $uniqueString = $this->generateUniqueString();
                            $newBaseUrl   = url('/') . '/v1/' . $uniqueString;
                            $updatedMessage = preg_replace(
                                '/https?:\/\/[\w\-\.]+(\/[^\s]*)?/', 
                                $newBaseUrl,
                                $data['message']
                            );
                            ...
                            $data['track_token'] = $uniqueString;
                            $array = json_decode($data['original_string'], true);
                    ...
                    foreach ($chunk as $key => $value) {
                        ...
                        $data = array_merge($data, $default);
                            ...
                            // if ($data['id']) {
                                ...
                                // $metaDataArry = $campaign->meta_data;
                                // $previousGroupIds = $metaDataArry['contact']['group_ids'];
                                // $groupCounts = [];
                            
                                // if (!empty($previousGroupIds)) {
                                    // foreach ($previousGroupIds as $groupId) {
                                        // Collect all contact IDs for each group
                                        // $groupCounts = array_merge($groupCounts, Contact::where('group_id', $groupId)->pluck('id')->toArray());
                                    // }
                                // }
                            
                                // $idToRemove = $data['meta_data']['id'];
                                // $groupCounts = array_diff($groupCounts, [$idToRemove]);
                            
                                // Check if this is the last index and then delete
                                // if ($key === $lastIndex) {
                                //     foreach ($groupCounts as $contactId) {
                                //         CommunicationLog::where('contact_id', $contactId)->where('campaign_id', $data['id'])->delete();
                                //     }
                                // }
                            // }
