                              ...
                              <div class="col-12">
                                <div class="form-inner">
                                  ...
                                  <select class="form-select select2-search" id="gateway_id" data-placeholder="{{ translate("Select a gateway") }}" data-show="5" aria-label="gateway_id" name="gateway_id">
                                    <option value=""></option>
                                    <option value="0"  {{  $selectedGateway['gateway_id'] == 0 ? 'selected' : '' }} >{{ translate("Random Rotation") }}</option>
                                    <option value="-1"  {{  $selectedGateway['gateway_id'] == -1 ? 'selected' : '' }} >{{ translate("Automatic") }}</option>
                                    @foreach($gateways as $type => $gateway)
                                        <optgroup label="{{ ucfirst($type) }}">
                                            @foreach($gateway as $id => $name)
                                                <option value="{{ $id }}" {{  $selectedGateway['gateway_id'] == $id ? 'selected' : '' }}>{{ $name }}</option>
                                            @endforeach
                                        </optgroup>
                                    @endforeach
                                  </select>
                    ...
                    <div class="form-element">
                        ...
                        <div class="col-xxl-8 col-xl-9">
                            ...
                            <div class="input-group">
                                ...
                                <option value="-1" disabled {{ is_null($selectedRepeatFormat) ? 'selected' : '' }}>{{ translate("Select a repeat format") }}</option>
                                @foreach(\App\Enums\CampaignRepeatEnum::toArray() as $key => $value)
                                    <option value="{{ $value }}" {{ $selectedRepeatFormat == $value ? 'selected' : '' }}>
                                        {{ ucfirst(transformToCamelCase($key)) }}
                                    </option>
                                @endforeach
                                </select>
                            </div>
