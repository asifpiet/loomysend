                ...
                <div class="col-md-5">
                    <div class="page-header-left">
                        ...
                        <nav aria-label="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page"> {{ translate("Edit Campaign") }} </li>
                            </ol>
                        </nav>
                        </div>
                    </div>
                            ...
                            <h5 class="form-element-title">{{ translate('From Group')}}</h5>
                                ...
                                <div class="row gy-3 align-items-end">
                                    ...
                                    <div class="form-inner">
                                        ...
                                        <select class="form-control" id="contacts" data-placeholder="{{ translate('Choose groups') }}" aria-label="contacts" name="contacts[]" multiple>
                                            <option value=""></option>
                        ...
                        <div class="col-xxl-8 col-lg-9">
                            ...
                            <div class="col-12">
                                ...
                                <select class="form-select select2-search" id="chooseMethod" data-placeholder="{{ translate("Choose a sending method") }}" aria-label="chooseMethod" name="method">
                                    <option value=""></option>
                                    <option value="{{ \App\Enums\StatusEnum::FALSE->status() }}"  {{ $selectedGateway['method'] == \App\Enums\StatusEnum::FALSE->status() ? 'selected' : '' }} >{{ translate("Android Gateway") }}</option>
                                    <option value="{{ \App\Enums\StatusEnum::TRUE->status() }}" {{$selectedGateway['method'] == \App\Enums\StatusEnum::TRUE->status() ? 'selected' : '' }} >{{ translate("API Gateway") }}</option>
                                </select>
                                </div>
                            </div>
...
if (selectedValues) {
    ...
    // Define the function
        ...
        if (method == "{{ \App\Enums\StatusEnum::FALSE->status() }}") {
            ...
            apiGatewayTypeSelect.change(function() {
                ...
                console.log("ApiGateway", selectedType);
                if (selectedType == '-1') {
    ...
    // Run the function on `chooseMethod` change
            $('#chooseMethod').change(updateGatewayOptions);
            
        });
    
    function initializeAndroidGateway() {
        var androidGateways = @json($android_gateways);  // Assuming this variable is available

        // Populate options for androidGatewaySelect
        var androidGatewayOptions = [
            { value: '-1', text: 'Automatic' }
        ];
        ...
        var androidGatewaySelect = $('#androidGatewaySelect'); // Select element
        androidGatewaySelect.empty(); // Clear previous options

        // Append options to androidGatewaySelect
        $.each(androidGatewayOptions, function(index, option) {
            androidGatewaySelect.append(new Option(option.text, option.value));
        });

        // Set a default value if one exists (e.g., pre-selected from controller)
        var defaultValue = "{{ $selectedGateway['gateway_id'] ?? '-1' }}"; // Replace with actual variable if available
        androidGatewaySelect.val(defaultValue);

        // Trigger change event to handle SIM options if a default value is set
        androidGatewaySelect.trigger('change');
    }
    function initializeApiGatewayType() {
        var apiGateways = @json($api_gateways);  // Assuming this variable is available

        // Set a default value if one exists (e.g., pre-selected from controller)
        var defaultApiGatewayType = "{{ $selectedGateway['gateway_id'] ?? '-1' }}"; // Replace with actual variable if available
        
        console.log("defaultApiGatewayType",defaultApiGatewayType);
        $('#apiGatewayTypeSelect').val(defaultApiGatewayType);

        // Trigger the change event to handle API Gateway options setup
        $('#apiGatewayTypeSelect').trigger('change');
    }

    // Initialize API Gateway Type options on page load
    

    // Change event handler for apiGatewayTypeSelect
    // $('#apiGatewayTypeSelect').change(function() {
    //     var selectedType = $(this).val();
    ...
    // });
        ...
        $('form').submit(function() {
            ...
            console.log("apiGatewayTypeSelectField",apiGatewayTypeSelectField, selectApiGateway);
            $('#gateway_id_manual').remove(); 
            $('#gateway_id_automatic').remove(); 
